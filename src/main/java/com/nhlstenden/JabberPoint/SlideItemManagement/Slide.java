...
import com.nhlstenden.JabberPoint.PresentationManagement.Observer;
...
public class Slide implements Observer {
  ...
  public void draw(Graphics g, Rectangle area, ImageObserver view) {
    ...
    BasicStyle style = BasicStyle.getStyle(slideItem.getLevel());
    slideItem.draw(area.x, y, scale, g, style, view);
    y += slideItem.getBoundingBox(g, view, scale, style).height;
    for (int number = 0; number < getSize(); number++) {
      slideItem = getSlideItems().elementAt(number);
      style = BasicStyle.getStyle(slideItem.getLevel());
      slideItem.draw(area.x, y, scale, g, style, view);
      y += slideItem.getBoundingBox(g, view, scale, style).height;
    }
  }
